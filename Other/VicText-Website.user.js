...
(async function () {
	...
	let hasCollections = document.querySelectorAll('div.col-md-4.col-sm-4:not(.item):not(.extraImageThumb)')?.length > 0 ?? false;
...
function addElementToDropdownContainer(locationElement, elementsToAdd, location = 'beforeEnd', showIf = true) {
	...
	outerContainer.insertAdjacentElement('beforeEnd', divider.cloneNode(true));
	outerContainer.querySelectorAll(':scope > .productDetailDivider' + (pagerForm ? '' : ':not(:last-of-type)')).forEach(elem => elem.remove());
}
...
.galleryImage {
	...
	MyStyles.addStyle('CenterImages', cssText, true); // Disabled by Default
...
#SearchTags {
	...
	MyStyles.addStyle('ShowSearchTags', cssText);
...
function addPagerOptionsAtTop(pagerWrappers) {
	...
	if (!loc.isBlogPage) pagerContainer.append(divider.cloneNode(true));
	...
	if (loc.isBlogPage) {
		loc.targetElem.before(pagerContainer);
	}
	else {
	...
	if (!cachedImagelessCollection || !cachedImagelessCollection[doc] || now - cachedImagelessCollection[doc]['timestamp'] > 5000) {
		...
		for (const currentItem of collection) {
			...
			if (currentImage.getAttribute('src').includes('NoImage.gif')
				...
				|| (currentImage.naturalHeight > 0 && currentImage.naturalHeight < _compareSize)) {
...
function addScraperOptions() {
	...
	table.classList.add('pull-right');
