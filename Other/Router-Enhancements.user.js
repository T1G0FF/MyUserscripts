...
function addMiscCSS() {
	let cssText = '';
	cssText = `/* Always show all table rows */
table.pure-table tr[style*="display: none;"] {
...
function addTableSorting() {
	let tables = document.querySelectorAll('table.pure-table');
	if (!tables || tables.length == 0) return;

	for (const table of tables) {
		let headerRow = table.querySelectorAll('th.table-head');
		let index = 0;
