...
(function () {
	...
	setInterval(function () {
		if (!document.querySelector('span.tg-dropdown-container')) {
			let locationElement = document.querySelector('p#toolbar-amount');
			createButtons(locationElement, 'beforeBegin');
			//addSortFilterInputs();
		}
...
function addMiscCSS() {
	let cssText = '';
	cssText = `/* Enable click through hidden bottom toolbar */
div.toolbar-bottom {
	pointer-events: none;
}`;
	MyStyles.addStyle('ClickThroughBottomToolbar', cssText);
}
...
function getItemObject(itemElement) {
	...
	if (/^For [0-9]+ Kits/g.test(givenDesc)) return; // Ignore Kits
